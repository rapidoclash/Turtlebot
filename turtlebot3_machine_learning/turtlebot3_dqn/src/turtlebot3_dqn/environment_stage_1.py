...
class Env():
    ...
    def getState(self, scan):
        ...
        # range à laquelle il detecte les collision
        if min_range > min(scan_range) > 0:
            done = True

        current_distance = round(math.hypot(self.goal_x - self.position.x, self.goal_y - self.position.y), 10) 
        if current_distance < 0.35:  # cercle de detection autour du carre
            self.get_goalbox = True

        return scan_range + [heading, current_distance], done


    # fct pour donner les recompenses au robot
    def setReward(self, state, done, action):
        yaw_reward = []
        current_distance = state[-1] # distance à l'objectif
        heading = state[-2] # orientation par rapport à l'objectif
        ...
        # si touche un mur ou obstacle
        if done:
            rospy.loginfo("Collision!!")
            reward = -1000
            self.pub_cmd_vel.publish(Twist())

        # si touche le carre rouge
        if self.get_goalbox:
            rospy.loginfo("Goal!!")
            reward = 2000
            self.pub_cmd_vel.publish(Twist())
    ...
    def step(self, action):
        ...
        #self.lastDist = self.getGoalDistace()
